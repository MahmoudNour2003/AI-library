        ...
        if should_process:
            ...
            with st.spinner("Processing PDF..."):
                try:
                    ...
                    prompt = f"""Extract the following metadata from this academic text 
                    and return ONLY valid JSON (no explanation):\n- title\n- authors (full names)\n\nText:\n{text}"""
    ...
    # ============ Image Workflow ============
        ...
        if uploaded_image:
            ...
            try:
                ...
                # Minimal TEI (since we only have metadata from image OCR)
                tei = "<TEI></TEI>"
                ...
                st.info("Generating MARC records...")
                record, marc_bin_path, marc_txt_path, marc_xml_path = tei_to_marc(
                    tei, marc_bin_path, temp_image_path, llm_metadata
                )
                if os.path.exists(marc_xml_path):
                    shutil.copy2(marc_xml_path, xml_db_path)

                st.success("MARC records generated successfully!")
                st.subheader("Extracted Metadata")
